...
function createABCFromGrooveData(track, renderWidth) {
	...
	editor.track.noteMappingArray = editorClickable.create_noteMappingArray_for_highlighting(FullNoteHHArray,
			FullNoteSnareArray,
			FullNoteKickArray,
			FullNoteTomsArray,
			FullNoteHHArray.length);
...
function generate_ABC(renderWidth) {
	...
	switch (editor.class_permutation_type) {
		case "kick_16ths": // use the hh & snare from the user
			...
			fullABC = get_top_ABC_BoilerPlate(editor.class_permutation_type != "none", tuneTitle, tuneAuthor, tuneComments, showLegend, usingTriplets(), false, editor.track.numBeats, editor.track.noteValue, renderWidth, editor.track.trackID);
			...
			for (i = 0; i < numSections; i++) {
				if (shouldDisplayPermutationForSection(i)) {
					...
					editor.track.noteMappingArray = editor.track.noteMappingArray.concat(editorClickable.create_noteMappingArray_for_highlighting(HH_Array, Snare_Array, new_kick_array, Toms_Array, num_notes));
				}
			}
			break;
		...
		case "snare_16ths": // use the hh & kick from the user
			...
			for (i = 0; i < numSections; i++) {
				if (shouldDisplayPermutationForSection(i)) {
					...
					editor.track.noteMappingArray = editor.track.noteMappingArray.concat(editorClickable.create_noteMappingArray_for_highlighting(HH_Array, new_snare_array, Kick_Array, Toms_Array, num_notes));
				}
			}
			break;
		...
		/* falls through */
		default:
			fullABC = get_top_ABC_BoilerPlate(editor.class_permutation_type != "none", tuneTitle, tuneAuthor, tuneComments, showLegend, usingTriplets(), true, editor.track.numBeats, editor.track.noteValue, renderWidth, editor.track.trackID);
			editor.track.noteMappingArray = [];
			...
			for (i = 0; i < editor.track.numberOfMeasures; i++) {
				...
				editor.track.noteMappingArray = editor.track.noteMappingArray.concat(editorClickable.create_noteMappingArray_for_highlighting(HH_Array, Snare_Array, Kick_Array, Toms_Array, num_notes));
